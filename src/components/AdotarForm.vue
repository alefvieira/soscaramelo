    ...
    <v-container>
        ...
        <v-row class="text-center" v-show="!hidden">
            <v-col offset-lg="1" lg="10" ms="12">
                <div v-if="listaAnime.length == 0">
                    <h1>Não foi encontrado nenhum registro!</h1>
                </div>

                <div
                    class="bloco--anime"
                    v-for="(anime, indice) in listaAnime"
                    :key="indice"
                >
                    <v-card class="mx-auto">
                        ...
                        <v-img height="150" :src="anime.imgcapa">
                        ...
                        <v-card-text>
                            ...
                            <div>
                                Disponível:
                                <strong>{{
                                    converteListaString(anime.plataformas)
                                }}</strong>
                                <br />
                                Com {{ anime.qtdtemporadas }} temporadas e
                                {{ anime.qtdEp }} de episódios.<br />Categorias:
                                <strong>{{
                                    converteListaString(anime.categorias)
                                }}</strong>
                                <br />Avaliação:
                            </div>
                        </v-card-text>
    ...
    methods: {
        async searchAnime() {
            const req = await fetch(
                "http://localhost:3000/anime?q=" + this.search
            );
        ...
        setCategoria() {
            if (
                !this.anime.categorias.includes(this.addCategoria) &&
                this.addCategoria != ""
            ) {
        ...
        async salvarAnime() {
            if (this.indice < 0) {
                ...
                const req = await fetch(
                    "http://localhost:3000/anime/" + this.indice,
                    {
                        method: "PUT",
                        body: objData,
                        headers: {
