            ...
            <v-col offset-lg="2" lg="8" md="12">
                <v-form v-model="valid">
                  <div class="input-form">
                    ...
                    <v-row>
                        <v-col cols="12">
                            <v-btn v-if="!loading" class="botao-01" @click="salvarUsuario">SALVAR</v-btn>
...
export default {
  ...
  methods: {
      ...
      perfilUsuario() {
        let objJson = JSON.parse(localStorage.getItem("usuario"));

        this.indice = objJson[0].usuario.id;
        this.usuario =  {
            name: objJson[0].name,
            email: objJson[0].email,
            perfil_id: objJson[0].perfil_id,
            telefone: objJson[0].usuario.telefone,
            endereco: objJson[0].usuario.endereco,
            dt_nascimento: objJson[0].usuario.dt_nascimento,
            cep: objJson[0].usuario.cep,
            cidade: objJson[0].usuario.cidade,
            bairro: objJson[0].usuario.bairro,
        }
      },
      ...
      async salvarUsuario() {
        ...
        this.loading = true;
        if(this.erro === "" && localStorage.getItem("usuario") && sessionStorage.getItem("token")){
          ...
          const req = await fetch('https://api-helpet.herokuapp.com/api/usuario/'+ this.indice,
          {
              method: "POST",
              body: objData,
              headers: { 
